...
## 2026-09-09 | 실험 루프 → DECISIONS 승격 다리 및 이중 축 관리

- 결정: 자동 실험 루프가 만든 keep 발견을 두 축으로 분류해 관리한다. **(A) 표준 레시피 축** = 고정 아키텍처, 크로스 벤더 비교의 기준선. **(B) 예산 특화 축** = 고정 시간 예산(5분) 하 스텝 수/스케일 최적화. 승격 규칙: (1) 개별 하이퍼파라미터 튠은 DECISIONS로 올리지 않고 리포트/실험 브랜치에 보존한다. (2) DECISIONS 승격 대상은 축과 무관하게 **재현되고 방향성이 확정된 구조적 결정**만. (3) 표준 레시피 축의 keep이 기준선을 갱신하면 VALIDATION의 표준 레시피 행 + CHANGELOG을 갱신한다. (4) 예산 특화 축은 VALIDATION의 별도 스냅샷 행으로만 관리하고 크로스 벤더 비교에는 쓰지 않는다. (5) noise 범위 내 개선은 승격하지 않는다.
- 이유: 자동 루프의 이득이 대부분 배치 축소→스텝 수 증가(예산 특화)에 몰려, 이를 기준선에 섞으면 "단일 소스의 벤더별 효율 격차" 측정이 오염된다. 2026-08-30 무인 커밋 결정에서 보류했던 "승격 다리"를 루프 구동 성공(감독 1회 + 밤샘 1회) 후 설계.
- 영향 범위: DECISIONS/VALIDATION/CHANGELOG 운영, 실험 회수 루틴
- 대안/보류: 예산 특화 축 최적점을 표준 레시피로 승격하는 것은 크로스 벤더 축이 예산 고정 비교로 재정의될 때만 재검토
